...
class NetboxAsInventory(object):
    ...
    def __init__(self, script_args, script_config_data):
        ...
        self.api_token = self._config(
            ["main", "api_token"], default="", optional=True)
    ...
    def get_hosts_list(api_url, api_token=None, specific_host=None):
        ...
        while api_url:
            ...
            api_output = requests.get(
                api_url, params=api_url_params, headers=api_url_headers)
            ...
            for category in groups_categories:
                ...
                if groups_categories[category]:
                    ...
                    for group in groups_categories[category]:
                        ...
                        if data_dict:
                            group_value = self._get_value_by_path(
                                data_dict, [group, key_name])

                            if group_value:
                                inventory_dict = self.add_host_to_group(
                            ...
                            # If any groups defined in "group_by" section, but host is not part of that group, it will go to catch-all group.
                                ...
                                self._put_host_to_ungrouped(
                                    inventory_dict, server_name)
        ...
        if host_vars:
            ...
            for category in host_vars:
                ...
                for var_name, var_data in host_vars[category].items():
                    ...
                    if var_data == "custom_fields":
                        var_value = data_dict.get(var_data)
                        if var_value and var_value.get('hardware_series'):
                            hs = var_value['hardware_series']
                            image = var_value['image']
                            var_value = {"hs": hs, "image": image}
                        else:
                            var_value = None
                    elif var_data == "config_context":
                        var_value = data_dict.get(var_data)
                        ...
                        var_value = self._get_value_by_path(
                            data_dict, [var_data, key_name], ignore_key_error=True)
                    else:
                        var_value = data_dict.get(var_data)
    ...
    def generate_inventory(self):
        ...
        netbox_hosts_list = self.get_hosts_list(
            self.api_url, self.api_token, self.host)
        ...
        if netbox_hosts_list:
            ...
            for current_host in netbox_hosts_list:
                ...
                self.add_host_to_inventory(
                    self.group_by, inventory_dict, current_host)
                host_vars = self.get_host_vars(current_host, self.hosts_vars)
                inventory_dict = self.update_host_meta_vars(
                    inventory_dict, server_name, host_vars)
        return inventory_dict
